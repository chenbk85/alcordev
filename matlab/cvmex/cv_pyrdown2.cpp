					...
					*prhs[])
	{
      ...
      if(nrhs > 0)
      {
        ...
        if(mxGetClassID(prhs[0]) == mxSINGLE_CLASS)
        {
          ...
          IplImage* p_downsampled32= cvCreateImage(  cvSize(width,height),
                                                    IPL_DEPTH_32F, 
                                                    1);

          cvPyrDown( p_image, p_downsampled32);

          if(nlhs > 0)
          {
            //
            plhs[0] = matlab::mxcv::iplimage_to_mxarray<core::single_t>(p_downsampled32);
          }
          core::cvutils::release_iplimage(p_image);
          core::cvutils::release_iplimage(p_downsampled32); 
       }
      }
      else
      {
